...
- 工具栏
  - 实现漂浮状态，即固定在制定个的位置   （_**<font color="blue">#DONE</font>**_）
- 节点
  - 拖动时，如到底部页面随之进行变动（滚动条变动）
...
- 添加文件 *WeScreen* 用于工作流布景管理
- **node**
  - (+) *添加新的可选属性`sets`用于将`c` 和 `label`等组合为集合*
- **WorkerEditor**
  - (修复) *采用 `sets` 属性，优化工具栏生成节点使“文字”和“容器”融为一体而修复文本选择无效的问题*
  - (调整) *初始化(config)配置文件调整， 删除与工具栏相关的“**图标配置**”，新增“**el_toolbar**”用于配置工具栏容器选择器*
  - (实现) *工具点击时的，清空所有正在选中的节点*
- **ToolBar**
  - (实现) *删除旧版“工具栏”生成规则，采用新的生成规则*
  - (调整) *删除工具栏中旧的方法`cToggle,tToggle`，使用jQuery的特性重写`show,hide`方法*
  - (实现) *通过“工具栏”与“工作流编辑器”与自之分离实现“fixed”工具栏，即不随着滚动条的滚动而滚动*
  - (+) *添加 `onClick` 接口，用于实现在工具栏生成时点击时间的外部调用接口*
  - **[fixed]**
    - _(修复) *由于工具栏父级`svg>div`等遮挡的原因导致节点在遮挡区而无法选中*_
    - _(修复)  方法`\_withoutElCfgThenCrtEl` 工具栏生成错误_
  - **[optimize]**
    - _(优化) 删除工具栏点击时显隐控制代码，而用独立后的方法代替_
  - **[new]**
    - _(新增) 方法 `toggleTools` 用于解耦工具栏点击事件的绑定代码_
    - _(新增) 内部添加设计文档，便于通过代码直接可看出设计理由_
    - *(新增) 方法 `_withoutElCfgThenCrtEl` 实现没有配置“工具栏”一样生成工具栏，使其与旧版兼容*
